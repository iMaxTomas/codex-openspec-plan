...
当前已有两类输入材料。第一类是本地讨论稿：[`del-ur-CONFIG.md`](/home/imax/document/test/codex-obs/del-ur-CONFIG.md) 讨论了 repository-level context files 经常因为冗余、显而易见、错误引导而伤害 agent 表现；[`AGENT-TALK.md`](/home/imax/document/test/codex-obs/AGENT-TALK.md) 则补充了 Hacker News 讨论中的实践侧视角，强调高质量、针对性的人工上下文在大项目中仍可能有价值。第二类是本地已下载的 arXiv TeX Source：[`2602.11988-TeX-Source.tar.gz`](/home/imax/document/test/codex-obs/2602.11988-TeX-Source.tar.gz) 与 [`2602.12670-TeX-Source.tar.gz`](/home/imax/document/test/codex-obs/2602.12670-TeX-Source.tar.gz)。
...
### 3. 把 `read-arxiv-paper` 作为首选阅读工具，而不是自己手写解析器
...
- [风险] 工具安装本身偏离主题 -> 缓解：只把 `read-arxiv-paper` 作为首个最小阅读工具
- [风险] 研究成果停留在文档，不进入可复用资产 -> 缓解：在 tasks 中显式要求回写成可复用 OpenSpec 计划

## Migration Plan

1. 安装或确认 `read-arxiv-paper`
2. 从两篇本地讨论稿提炼初始框架
...
- `read-arxiv-paper` 是否应被正式纳入长期研究 skill 栈
