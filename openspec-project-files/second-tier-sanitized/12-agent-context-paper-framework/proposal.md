...
当前需要把关于 `AGENTS.md / CLAUDE.md / skills / context management` 的零散讨论，整理成一个可复用的研究框架，而不是继续停留在经验判断。`<discussion-note-a.md>` 和 `<discussion-note-b.md>` 已经提供了问题意识，但还缺少基于论文源码的系统化验证与沉淀。

## What Changes

- 新增一个面向论文深读与框架沉淀的研究型 change，用于组织这次对 `Evaluating AGENTS.md` 与 `SkillsBench` 两篇论文的深入理解。
- 先从本地讨论稿 `<discussion-note-a.md>` 与 `<discussion-note-b.md>` 中提炼初始框架，明确“配置文件、技能模块、上下文管理、可复用工作流”的关键张力。
- 把安装并使用 `<paper-reading-skill>` 作为首个阅读工具接入动作，用于高质量读取本地 arXiv TeX Source。
